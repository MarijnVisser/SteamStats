...
@section('content')
    ...
    <div class="container">
        <div class="row bg-secondary">
            ...
            <div class="col-md-5">
                ...
                @endif
                <span class="userPageInfo">Level: <div id="divLevel"
                                                       class="friendPlayerLevelNum">{{ $gamedata['playerLevel']['player_level'] }}</div></span>
            ...
            <div class="col-md-4 pt-3">
                <p class="userPageInfo border border-dark">Vac ban: @if($gamedata['banInfo']['VACBanned'] == false)
                        None @else {{$gamedata['banInfo']['NumberOfVACBans']}} Ban(s)
                        ({{ $gamedata['banInfo']['DaysSinceLastBan'] }} Days ago) @endif</p>
                <p class="userPageInfo border border-dark">Community
                    ban: @if($gamedata['banInfo']['CommunityBanned'] == false) None @else Banned @endif</p>
                <p class="userPageInfo border border-dark">Game
                    bans: @if($gamedata['banInfo']['NumberOfGameBans'] == false)
                        None @else {{$gamedata['banInfo']['NumberOfGameBans']}} @endif</p>
                <p class="userPageInfo border border-dark">Trade ban: @if($gamedata['banInfo']['EconomyBan'] == 'none')
                        None @else {{$gamedata['banInfo']['EconomyBan']}} @endif</p>
                <p class="userPageInfo border border-dark">Account age: {{ date('y') - gmdate('y', $timeCreated) }}
                    years</p>
    ...
    <progress id="file" max="{{$gamedata['playerLevel']['player_xp_needed_current_level']}}"
              value="{{ $gamedata['playerLevel']['player_xp_needed_to_level_up']}}"></progress>
    ...
    <!--    Ban information     -->
    <p>Vac ban: @if($gamedata['banInfo']['VACBanned'] == false) None @else {{$gamedata['banInfo']['NumberOfVACBans']}}
        Ban(s) ({{ $gamedata['banInfo']['DaysSinceLastBan'] }} Days ago) @endif</p>
    <p>Community ban: @if($gamedata['banInfo']['CommunityBanned'] == false) None @else Banned @endif</p>
    <p>Game bans: @if($gamedata['banInfo']['NumberOfGameBans'] == false)
            None @else {{$gamedata['banInfo']['NumberOfGameBans']}} @endif</p>
    <p>Trade ban: @if($gamedata['banInfo']['EconomyBan'] == 'none')
            None @else {{$gamedata['banInfo']['EconomyBan']}} @endif</p>
    ...
    @foreach ($gamedata['recentlyPlayedGames'] as $recentlyPlayedGame)
        <img
            src="http://media.steampowered.com/steamcommunity/public/images/apps/{{ $recentlyPlayedGame['appid'] }}/{{ !empty($recentlyPlayedGame['img_logo_url']) ? $recentlyPlayedGame['img_logo_url'] : "No img"}}.jpg">
        <img
            src="http://media.steampowered.com/steamcommunity/public/images/apps/{{ $recentlyPlayedGame['appid'] }}/{{ !empty($recentlyPlayedGame['img_icon_url']) ? $recentlyPlayedGame['img_icon_url'] : "No img"}}.jpg">
        <p>Appid: {{ $recentlyPlayedGame['appid'] }}</p>
